...
function applyHit(opts = {}) {
  ...
  state.shake = state.equipped === 'grenade'
    ? 0.55
    : Math.min(0.5, 0.15 + state.combo * 0.008);
...
function shopRow(w) {
  ...
  if (w.id === 'wand') {
    ...
    if (state.equipped === 'wand') row.classList.add('equipped');
  } else if (lvLocked) {
    // レベル未達：完全シークレット
...
function animate() {
  ...
  pend.vx += (-(g / L) * Math.sin(anchor.rotation.x) - damp * pend.vx) * dt;
  pend.vz += (-(g / L) * Math.sin(anchor.rotation.z) - damp * pend.vz) * dt;
  anchor.rotation.x += pend.vx * dt;
  anchor.rotation.z += pend.vz * dt;
  pend.spinV += (-ojisan.rotation.y * 6 - pend.spinV * 2) * dt;
  ojisan.rotation.y += pend.spinV * dt;
  ...
  if (state.swinging) {
    ...
    if (equipCtx.anim === 'swing') {
      ...
      weaponRoot.rotation.x = br.x - ease * 1.75;
      weaponRoot.rotation.z = br.z + ease * 0.5;
      weaponRoot.position.z = bp.z - ease * 0.45;
